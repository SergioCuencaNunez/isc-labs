...
Both the mean and the std are very close to the expected values. It took longer to to generate the initial vector (`17.36s`) than to compute the mean (`0.21s`) and standard deviation (`1.50s`).
...
The function `lapply` took longer than the code in question 5b, which only needed 0.02s. This function works as a `for` loop, in comparison to the code in question 5b, which standardizes the vector using a vectorized approach.
...
Lists can store different type of data, which require more complex structures than vectors. For the current purpose It is not very useful to use lists, since all elements are integers. 
...
The `sum()` function uses an iterative approach, therefore it took 99 additions.
...
There is no clear relationship between the weight and the volume, even though it is expected to have a positive correlation between both. The correlation coefficient is a possible measure. In this case it shows a weak relationship, just 0.28.
...
The `=` assignment does not change the global value of `a`, only inside the function. On the other hand, `<-` changes the value both inside and outside of the function.
