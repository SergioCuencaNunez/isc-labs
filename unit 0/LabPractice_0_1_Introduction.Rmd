...
```
Expected mean is 7.5 (15 * 0.5), very close to the calculated mean.
Expected std is 1.9364916731037085 (sqrt(15 * 0.5 * (1 - 0.5))), very close as well to calculated std.
...
```
Tabulate counts the occurrences of integer values in the vector bin.draws.0.5. The output vector matches exactly the histogram, as hist only represents visually these values.
...
plot(bin.draws.0.5)
```
Plot represents every single element in the vector. It plots every element as a function of their index (x-axis) and value (y-axis).
...
```
== makes a pairwaise comparison of the value of every element in the vector. It outputs TRUE for every element, which is correct as they share the same values.

identical checks if both vectors are exactly the same, which is not the case as means is a vector and matrix.means a named vector. 
...
```{r q5b}
system.time((big.bin.draws - mean.big)/std.big)
...
system.time(lapply(big.bin.draws.list, standardize))
big.bin.draws.list.standardized.slow = lapply(big.bin.draws.list, standardize)
```
Lapply took longer than the code in Q5b, which only needed 0.02s. Lapply works as a for loop, in comparison to the code in Q5b, which standardizes the vector using a vectorized approach.
...
system.time(lapply(big.bin.draws.list, standardize.slow))
```
This code is much slower, because for the standardization of every element the mean and std of the complete vector is being newly calculated, as opposed to the previous code, where they were already calculated and used as a number.
...
vector: 20MB
List: 320MB
Lists can store different type of data, which require more complex structures than vectors. For the current purpose I do not think it is very useful to use lists, since all elements are integers. 
...
```{r q8a}
simData1 <- read.csv('./data/SimData1.csv', sep=';')
simData2 <- read.csv('./data/SimData2.csv', sep=';', header=TRUE)
simData2$Y <- factor(simData2$Y)
simData3 <- read.csv('./data/SimData3.csv', sep=';')
simData4 <- read.csv('./data/SimData4.csv', sep=';')
simData5 <- read.csv('./data/SimData5.csv', sep=';')
simData5$Y <- factor(simData5$Y)
simData6 <- read.csv('./data/SimData6.csv', sep=';')
simData7 <- read.csv('./data/SimData7.csv', sep=';')
simData7$Y <- factor(simData7$Y)
```

**8b** Does the data need preprocessing? Check if there are outliers or missing values. If any, how many are there?

```{r q8b}
outliers.11 <- boxplot.stats(simData1$X1)$out
outliers.12 <- boxplot.stats(simData1$Y)$out
na.1 <- sum(is.na(simData1))
outliers.21 <- boxplot.stats(simData2$X1)$out
outliers.22 <- boxplot.stats(simData2$X2)$out
na.2 <- sum(is.na(simData2))
outliers.31 <- boxplot.stats(simData3$X1)$out
outliers.32 <- boxplot.stats(simData3$Y)$out
na.3 <- sum(is.na(simData3))
outliers.41 <- boxplot.stats(simData4$X1)$out
outliers.42 <- boxplot.stats(simData4$X2)$out
outliers.43 <- boxplot.stats(simData4$Y)$out
na.4 <- sum(is.na(simData4))
outliers.51 <- boxplot.stats(simData5$X1)$out
outliers.52 <- boxplot.stats(simData5$X2)$out
na.5 <- sum(is.na(simData5))
outliers.61 <- boxplot.stats(simData6$X1)$out
outliers.62 <- boxplot.stats(simData6$Y)$out
na.6 <- sum(is.na(simData6))
outliers.71 <- boxplot.stats(simData7$X1)$out
outliers.72 <- boxplot.stats(simData7$X2)$out
na.7 <- sum(is.na(simData7))
```
...
```{r q8c}
library(ggplot2)
ggplot(data = data.frame(x = simData1$X1, y = simData1$Y), aes(x = simData1$X1, y = simData1$Y))+
  geom_point(size = 2)+
  labs(title = "Scatterplot simData1",
       x = "X1",
       y = "Y")
ggplot(data = data.frame(x = simData2$X1, y = simData2$X2), aes(x = simData2$X1, y = simData2$X2, color = simData2$Y)) +
  geom_point(size = 2) +
  labs(title = "Scatterplot simData2",
       x = "X1",
       y = "X2",
       color = "Y")
ggplot(data = data.frame(x = simData3$X1, y = simData3$Y), aes(x = simData3$X1, y = simData3$Y))+
  geom_point(size = 2)+
  labs(title = "Scatterplot simData3",
       x = "X1",
       y = "Y")
ggplot(data = data.frame(x = simData4$X1, y = simData4$X2), aes(x = simData4$X1, y = simData4$X2, color = simData4$Y)) +
  geom_point(size = 2) +
  labs(title = "Scatterplot simData4",
       x = "X1",
       y = "X2",
       color = "Y")
ggplot(data = data.frame(x = simData5$X1, y = simData5$X2), aes(x = simData5$X1, y = simData5$X2, color = simData5$Y)) +
  geom_point(size = 2) +
  labs(title = "Scatterplot simData5",
       x = "X1",
       y = "X2",
       color = "Y")
ggplot(data = data.frame(x = simData6$X1, y = simData6$Y), aes(x = simData6$X1, y = simData6$Y))+
  geom_point(size = 2)+
  labs(title = "Scatterplot simData6",
       x = "X1",
       y = "Y")
ggplot(data = data.frame(x = simData7$X1, y = simData7$X2), aes(x = simData7$X1, y = simData7$X2, color = simData7$Y)) +
  geom_point(size = 2) +
  labs(title = "Scatterplot simData7",
       x = "X1",
       y = "X2",
       color = "Y")
```
...
```{r q8d}
model_lm_1 <- lm(simData1$Y ~ ., data = as.data.frame(simData1$X1))
model_lm_1$coefficients
model_glm_2 <- glm(Y ~ ., data = simData2, family = "binomial")
coef(model_glm_2)
model_lm_3 <- lm(simData3$Y ~ ., data = as.data.frame(simData3$X1))
model_lm_3$coefficients
model_glm_4 <- glm(Y ~ ., data = simData4, family = "binomial")
coef(model_glm_4)
model_glm_5 <- glm(Y ~ ., data = simData5, family = "binomial")
coef(model_glm_5)
model_lm_6 <- lm(simData6$Y ~ ., data = as.data.frame(simData6$X1))
model_lm_6$coefficients
model_glm_7 <- glm(Y ~ ., data = simData7, family = "binomial")
coef(model_glm_7)
```
...
ggplot(simData7, aes(x = Y)) +
  geom_bar(fill = c("lightyellow","violet", "lightcoral"), color = "black") +  # Fill and border colors
  labs(title = "Y", x = "Factor", y = "Frecuencia")
...
```{r q8f}
ggplot(simData7, aes(x = X1)) +
  geom_histogram(fill = "lightblue", color = "black") +
  facet_wrap(~ Y) + 
  labs(title = "X1 by Y", x = "Values of X1", y = "Frequency")

ggplot(simData7, aes(x = X2)) +
  geom_histogram(fill = "lightgreen", color = "black") +
  facet_wrap(~ Y) + 
  labs(title = "X2 by Y", x = "Values of X2", y = "Frequency")
```
...
ggpairs(simData7, 
        columns = c("X1", "X2"),
        mapping = aes(color = Y),
        title = "Pairwise Relationships in SimData7")
```
...
typeof(x.vec)
sum(x.vec)
(100/2)*(1+100) #Gauss' method
```
Sum() uses an iterative approach, therefore it took 99 additions.
...
```{r q10d}
pros.data.sub <- pros.dat[, c("lcavol", "lweight")]
colnames(pros.data.sub)
head(pros.data.sub,6)
```
...
```{r q10e}
log.cancer.density.1 <- mapply(function(x, y) x / y, pros.data.sub[, 2], pros.data.sub[, 1])
log.cancer.density.2 <- pros.data.sub[,2]/pros.data.sub[,1]
all.equal(log.cancer.density.1, log.cancer.density.2)
```
...
```{r q10f}
pros.dat<- cbind(pros.dat, log.cancer.density.1)
dim(pros.dat)
...
```{r q11a}
hist(pros.dat[,'lcavol'], main='Log Cancer Volume', breaks = 20, xlab='Log Volume')
hist(pros.dat[,'lweight'], main='Log Cancer Weight', breaks = 20, xlab='Log Weight')
plot(pros.dat[,'lweight'],pros.dat[,'lcavol'], main='Log cancer volume vs Log cancer weight',xlab='Log Weight', ylab='Log Volume')
cor(pros.dat[,'lweight'],pros.dat[,'lcavol'])
```
Log cancer weight follows more or less a normal distribution with a skew towards higher values. 
Log cancer volume has no clear distribution of the values.
There is no clear relationship between the weight and the volume, even though I expected there to be a positive correlation between both.
The correlation coefficient is a possible measure. In this case it shows a weak relationship, just 0.28.
...
```{r q11b}
plot(pros.dat[,'age'], pros.dat[,'lweight'], main='Age vs Log cancer weight',xlab='Age', ylab='Log Weight')
plot(pros.dat[,'age'],pros.dat[,'lcavol'], main='Log cancer volume vs Age',xlab='Age', ylab='Log Volume')
```
Between age and weight a slight positive upward trend may be inferred. 
Between age and volume there is no intuitive relationship. For both cases the amount of cases is more concentrated on higher ages.
...
```{r q11c}
hist(pros.dat[,'ldens'], main='Log Cancer Density', breaks = 20, xlab='Log Density')
plot(pros.dat[,'age'],pros.dat[,'ldens'], main='Log cancer density vs Age',xlab='Age', ylab='Log Density')
# Histogram without outliers
ldens_no_outliers <- pros.dat[, 'ldens']
q <- quantile(ldens_no_outliers, probs = c(0.25, 0.75))
iqr <- q[2] - q[1]
upper_bound <- q[2] + 1.5 * iqr
lower_bound <- q[1] - 1.5 * iqr
ldens_no_outliers <- ldens_no_outliers[ldens_no_outliers >= lower_bound & ldens_no_outliers <= upper_bound]

hist(ldens_no_outliers, main = 'Log Cancer Density (No Outliers)', breaks = 20, xlab = 'Log Density')
# Scatterplot without outliers
plot(pros.dat[pros.dat[, 'ldens'] >= lower_bound & pros.dat[, 'ldens'] <= upper_bound, 'age'], 
     ldens_no_outliers, 
     main = 'Log cancer density vs Age (No Outliers)', 
     xlab = 'Age', 
     ylab = 'Log Density')

```
The log density, as opposed to the log volume and log weight seems to follow a logarithmic distribution.
With respect to the age there is no clear pattern as how it relates to the log density, though the highest densities seem to be concentrated between 55 and 65.
...
```
Variables lcavol, lbph, svi, lcp and pgg45 have different means. 
...
pros.dat.df <- ((sx^2/n + sy^2/m)^2)/(((sx^2/n)^2/(n-1)) + ((sy^2/m)^2/(m-1)))
...
```{r q14c}
pt(pros.dat.t.stat, pros.dat.df)
```
...
repeat {
  var_name <- readline(prompt = "Enter var name to plot (or 'quit' to exit): ")

  if (var_name == "quit") {
    break  
  } else if (var_name %in% names(pros.dat)) {
    hist(pros.dat[, var_name], 
...
repeat {
  ...
  if (var_name == "quit") {
    ...
    plot_type <- readline(prompt = "Enter plot type (hist or scatter): ")
    ...
    if (plot_type == "hist") {
      ...
      if (ncol(pros.dat) > 2) {
        other_var <- readline(prompt = "Enter another variable for scatterplot: ")
        if (other_var %in% names(pros.dat) && other_var != var_name) {
          plot(pros.dat[, var_name], pros.dat[, other_var],
               main = paste(var_name, "vs.", other_var),
               xlab = var_name,
               ylab = other_var,
               col = "red",    
               pch = 16)  
        } else {
          print("Invalid second variable for scatterplot.")
        }
      } else {
        print("Scatterplots require at least two variables.")
      }
    } else {
      print("Invalid plot type. Choose 'hist' or 'scatter'.")
    }
...
state.df$Cetner.y <- state.center$y
head(state.df,3)
```
...
```{r q17d}
state.df.filtered <- state.df[state.df$Center.x< -100,]
state.df.subset <- subset(state.df, Center.x< -100)
all.equal(state.df.filtered, state.df.subset)
```
...
```{r q17e}
state.df.new <- state.df[state.df$Center.x< -100 & state.df$Murder > 9, ]
print(state.df.new)
state.df.new[which.max(state.df.new$Life.Exp),]
```
...
```{r q18b}
mapply(function(x, name) print(qplot(pros.dat$svi, x, ylab = name, xlab = "svi")), pros.dat[, names(pros.dat) != "svi"], names(pros.dat)[names(pros.dat) != "svi"])
```
...
```{r q18c2}
tests <- lapply(pros.dat[, names(pros.dat) != "svi"], t.test.by.ind, ind = pros.dat$svi)
...
print(sum(is.na(rio)))
```
Each row represents one of the participants in the Rio Olimpic Games.
...
print(nrow(rio))
print("Amount of Countries")
print(length(unique(rio[["nationality"]])))
print("Athletes per Country")
print(table(rio[["nationality"]]))
print("Country with most athletes")
print(names(table(rio[["nationality"]]))[which.max(table(rio[["nationality"]]))])
print(max(table(rio[["nationality"]])))
```
...
print(sum(rio[["gold"]]))
print("Silver")
print(sum(rio[["silver"]]))
print("Bronze")
print(sum(rio[["bronze"]]))
```
There is an unequal amount of medals for each type, which is likely due to ties in individual events. 
...
print(rio[rio[["total"]] == max(rio[["total"]]), ]) 
...
```{r q19f}
rio_zero_medals <- rio[rio[["nationality"]] %in% names(total.by.nat)[total.by.nat == 0], ]

names(table(rio_zero_medals[["nationality"]]))[which.max(table(rio_zero_medals[["nationality"]]))]
max(athletes_per_country)
...
```{r q20a}
rio$age <- 2016 - as.integer(substr(rio$date_of_birth, 1,4))
rio[rio$age == max(rio$age), ] # Oldest
rio[rio$age == min(rio$age), ] # Youngest
```
...
rio.medals[rio.medals$age == max(rio.medals$age), ] # Oldest
rio.medals[rio.medals$age == min(rio.medals$age), ] # Youngest
```

**20c.** Using a single call to `tapply()`, answer: how old are the youngest and oldest athletes, for each sport? 

```{r q20c}
sport.age <- tapply(rio.medals$age, rio.medals$sport, function(ages) {
  c(min = min(ages, na.rm = TRUE), max = max(ages, na.rm = TRUE))
})
print(sport.age)
```
...
```{r q20d}
sport.age.matrix <- matrix(unlist(sport.age), ncol = 2, byrow = TRUE)
rownames(sport.age.matrix) <- names(sport.age)
colnames(sport.age.matrix) <- c("Youngest", "Oldest")
print(sport.age.matrix)
```
...
```{r q20chal}
library(dplyr)
athletes.by.sport <- rio.medals %>%
  group_by(sport) %>%
  summarize(
    youngest.athlete = name[age == min(age, na.rm = TRUE)][1],
    youngest.age = min(age, na.rm = TRUE),
    oldest.athlete = name[age == max(age, na.rm = TRUE)][1],
    oldest.age = max(age, na.rm = TRUE)
  )

print(athletes.by.sport)
```
...
sports <- rio %>%
  ...
  summarize(
    n.participants = n(),
    n.gold = sum(gold),
    gold.ratio = n.gold / n.participants
  ) %>%
  arrange(sport) #alphabetical order
print("Highest ratio")
sports$sport[which.max(sports$gold.ratio)]
max(sports$gold.ratio)
print("Lowest ratio")
sports$sport[which.min(sports$gold.ratio)]
min(sports$gold.ratio)
...
```{r q21b}
sports$ave_weight <- tapply(rio$weight, rio$sport, mean, na.rm = TRUE)[sports$sport]
sports %>%
  arrange(desc(ave_weight)) %>%
  select(sport, ave_weight) %>%
  print()
```
...
```{r q21c}
sports$ave_weight_men <- tapply(rio$weight[rio$sex == "male"], rio$sport[rio$sex == "male"], mean, na.rm = TRUE)[sports$sport]
sports %>%
  arrange(desc(ave_weight_men)) %>%
  select(sport, ave_weight_men) %>%
  print()
sports$ave_weight_women <- tapply(rio$weight[rio$sex == "female"], rio$sport[rio$sex == "female"], mean, na.rm = TRUE)[sports$sport]
sports %>%
  arrange(desc(ave_weight_women)) %>%
  select(sport, ave_weight_women) %>%
  print()
```
...
```{r q21chal}
sports$prop_women <- tapply(rio$sex == "female", rio$sport, mean, na.rm = TRUE)[sports$sport]
sports$ave_weight2 <- sports$prop_women * sports$ave_weight_women + (1 - sports$prop_women) * sports$ave_weight_men

sports$ave_weight_men <- tapply(rio$weight[rio$sex == "M"], rio$sport[rio$sex == "M"], mean, na.rm = TRUE)[sports$sport]
sports$n_men <- tapply(rio$sex == "male", rio$sport, sum, na.rm = TRUE)[sports$sport]
sports$n_women <- tapply(rio$sex == "female", rio$sport, sum, na.rm = TRUE)[sports$sport]
sports$ave_weight_recreated <- (sports$n_men * sports$ave_weight_men + sports$n_women * sports$ave_weight_women) / (sports$n_men + sports$n_women)
...
print(paste("huber(1) =", huber(1)))
print(paste("huber(4) =", huber(4)))
```
...
print(paste("huber(3,2) =", huber(3,2)))
print(paste("huber(3,4) =", huber(3,4)))
```
...
print(paste("huber(3) =", huber(3)))
```

**22d.** Check that `huber(a=1, x=3)` returns 5. Check that `huber(1, 3)` returns 1. Explain why these are different.

```{r q22d}
print(paste("huber(3,1) =", huber(3,1)))
print(paste("huber(1,3) =", huber(1,3)))
```
The difference in both results lies in x and a not being interchangeable.
...
```{r q22e}
huber <- function(x, a = 5) {
  ifelse(abs(x) <= a, x^2, 2 * a * abs(x) - a^2)
}
print(paste("huber(1:6,3) =", huber(1:6,3)))
```
...
x.vals <- -20:20
a <- 6
huber.vals <- huber(x.vals, a)
plot(x.vals, huber.vals, type = "l", xlab = "x", ylab = "Huber Value",
     main = "Huber Function")
```
Visually you can infer that around -6 and 6 the curve turns into a straight line.
...
```{r q23a}
plot(x.vals, huber.vals, type = "l", 
     xlab = "x", ylab = "psi(x)", 
     main = "Huber",
     xlim = c(-20, 20), ylim = c(0, 200)) 

# Vertica dotted lines
abline(v = c(-6, 6), col = "red", lty = 2)
# Text
text(-15, 60, "Linear", cex = 1.3)
text(0, 60, "Quadratic", cex = 1.3)
text(15, 60, "Linear", cex = 1.3)
```
...
huber <- function(x, a = 5) {
  ...
  plot(c(0, 1), c(0, 1), type = "n",
       ...
       xlab = "", ylab = "",         
       main = "")                   
  rect(0, 0, 1, 1, col = "red", border = NA)  # Red background
  rect(0.4, 0.2, 0.6, 0.8, col = "white", border = NA) # Vertical bar
  rect(0.2, 0.4, 0.8, 0.6, col = "white", border = NA)
  ifelse(abs(x) <= a, x^2, 2 * a * abs(x) - a^2)
}
print(paste("huber(1:6,3) =", huber(1:6,3)))
```
...
```{r q24c}
a = 1.5
print(paste("huber.sloppy(3) =", huber.sloppy(3)))
a = 3
print(paste("huber.sloppy(3) =", huber.sloppy(3)))
a = "hi"
print(paste("huber.sloppy(3) =", huber.sloppy(3)))
```
The function uses the x value as a.
...
```{r q24d}
print(paste("huber(3 , a = 5) =", huber(3, a = 5)))
print(a)
print(paste("huber(3, a <- 5) =", huber(3, a <- 5)))
print(a)
```
= assignment does not change the global value of a, only inside the function. <- changes the value both inside and outside of the function.
...
```{r q24e}
print(paste("huber(3, b <- 20) =", huber(3, b <- 20)))
print(b)
```
The value of b is being ignored by the function, and the value of a is being used.
...
```{r q24chal1}
plot(x <- seq(0, 2, length.out = 50), 0.05 * x^2 - sin(x) * cos(x) + 0.1 * exp(1 + log(x)), type = "l")
```
...
print(paste("Does 'y' exist in the global environment?", exists("y")))
```
